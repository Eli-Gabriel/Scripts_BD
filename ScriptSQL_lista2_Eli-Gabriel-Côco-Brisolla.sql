...
#Parte 1
#
select cliente.nome_cli as 'Nome', 
cliente.cpf_cli as 'cpf', 
(select sexo.nome_sex from sexo where sexo.cod_sex = cliente.cod_cli) as 'Sexo',
...
on cliente.cod_end = endereco.cod_end inner join cidade
on endereco.cod_cid = cidade.cod_cid inner join estado
on cidade.cod_est = estado.cod_est;
###exibe o sexo de apenas 2 clientes

#2
select funcionario.cod_func as 'Código', 
funcionario.nome_func as 'Nome', 
funcionario.cpf_func as 'CPF', 
(select sexo.nome_sex from sexo where sexo.cod_sex = funcionario.cod_sex) as 'Sexo',
endereco.bairro_end as 'Bairro', 
endereco.numero_end as 'Número', 
cidade.nome_cid as 'Cidade', 
estado.nome_est as 'Estado',
funcionario.função_func as 'Função', 
(select departamento.nome_dep from departamento where departamento.cod_dep = funcionario.cod_dep) as 'Departamento'
from funcionario inner join endereco
on funcionario.cod_end = endereco.cod_end inner join cidade
on endereco.cod_cid = cidade.cod_cid inner join estado
on cidade.cod_est = estado.cod_est;
...
on cliente. cod_cli = carro.cod_car;

#4
select fornecedor.nomefantasial_forn as 'Nome Fantasia', 
compra_produto.cod_comp as 'Código', 
produto.descrição_prod as 'Descrição',
itens_compra.quant_itenscomp as 'Quantidade', 
itens_compra.valor_itenscomp as 'Valor'
from fornecedor inner join compra_produto
on fornecedor.cod_forn = compra_produto.cod_forn inner join itens_compra
on compra_produto.cod_comp = itens_compra.cod_comp inner join produto
on itens_compra.cod_prod = produto.cod_prod;

#5
select cliente.nome_cli as 'Nome Cliente', 
venda.cod_vend as 'Código Venda', 
produto.descrição_prod as 'Produto', 
itens_venda.quant_itensvend as 'Quantidade', 
itens_venda.valor_itensvend as 'Valor'
from cliente inner join venda
on cliente.cod_cli = venda.cod_cli inner join itens_venda
on venda.cod_vend = itens_venda.cod_vend inner join produto
on itens_venda.cod_prod = produto.cod_prod;

#6
select fornecedor.cod_forn as 'Código Fornecedor', 
fornecedor.nomefantasial_forn as 'Nome Forncedor',
count(compra_produto.cod_comp) as 'Quantidade compras', 
sum(compra_produto.valortotal_comp) as 'Valor'
from fornecedor inner join compra_produto 
on fornecedor.cod_forn = compra_produto.cod_forn;

#7
select cliente.cod_cli as 'Código Cliente', 
cliente.nome_cli as 'Nome Cliente',
count(venda.cod_vend) as 'Quantidade de compras', 
sum(venda.valortotal_vend) as 'Valor'
from cliente inner join venda 
on cliente.cod_cli = venda.cod_cli;

#8
select venda.cod_vend as 'Código Venda', 
venda.data_vend as 'Data Venda', 
venda.formpag_vend as 'Forma de Pagamento',
cliente.nome_cli as 'Cliente', 
(select count(itens_venda.cod_itensvend) from itens_venda where itens_venda.cod_vend = venda.cod_vend) as 'Quantidade',
(select sum(itens_venda.valor_itensvend) from itens_venda where itens_venda.cod_vend = venda.cod_vend) as 'Total'
from venda inner join cliente 
on venda.cod_cli = cliente.cod_cli;

#9
select pagamentos.cod_pag as 'Código', 
pagamentos.data_pag as 'Data', 
pagamentos.valor_pag as 'Valor', 
pagamentos.formapagamento_pag as 'forma de Pagamento',
caixa.cod_caixa as 'Código Caixa', 
caixa.dataabertura_caixa as 'Data Caixa', 
(select funcionario.nome_func, funcionario.função_func as 'Função' from funcionario where pagamentos.cod_func = funcionario.cod_func) as 'Funcionario', 
(select despesas.datavencimento_desp, despesas.valor_desp as 'Valor' from despesas where despesas.cod_desp = pagamentos.cod_desp)  as 'Data de Vencimento'
from pagamentos inner join caixa
on pagamentos.cod_caixa = caixa.cod_caixa;
#????????????????????????????????????

#10
select funcionario.nome_func as 'Funcionario', count(venda.cod_vend) as 'Quantidade', sum(venda.valortotal_vend) as 'Total'
from venda inner join funcionario on venda.cod_func = funcionario.cod_func;


#PARTE2
#1
#tem duas respostas porque o mysql no meu computador mostra um erro(diz que o update não é um input valido nesta posição ) 
#no primeiro update(independente de qual questão ou se ela esta certa ou errada)
update itens_compra set valor_itenscomp = valor_itenscomp*(select count(itens_compra.cod_prod) from produto where itens_compra.cod_prod = produto.cod_prod);
update itens_compra set valor_itenscomp = valor_itenscomp*(select count(itens_compra.cod_prod) from produto where itens_compra.cod_prod = produto.cod_prod);
...
#3
update produto set produto.quant_prod = produto.quant_prod - (select count(itens_compra.cod_prod) from itens_compra where itens_compra.cod_prod = produto.cod_prod);

#4
update venda set valortotal_vend = valortotal_vend * (select itens_venda.quant_itensvend from itens_venda where itens_venda.cod_vend = venda.cod_vend);
#??????????

#5
update itens_venda set valor_itensvend = valor_itensvend + (select sum(produto.valor_prod) from produto where produto.cod_prod = itens_venda.cod_prod);

#6
update produto set quant_prod = quant_prod - (select itens_venda.quant_itensvend from itens_venda where itens_venda.cod_prod = produto.cod_prod);
#????

#7
update pagamentos set valor_pag = valor_pag + (select despesas.valor_desp from despesas where despesas.cod_desp = pagamentos.cod_desp);

#8
update pagamentos set valor_pag = valor_pag + (select compra_produto.valortotal_comp from compra_produto where compra_produto.cod_comp = pagamentos.cod_comp);

#9
update recebimentos set valor_receb = valor_receb + (select venda.valortotal_vend from venda where venda.cod_vend = recebimentos.cod_vend);

#10
update caixa set valorcréditos_caixa = valorcréditos_caixa + (select sum(recebimentos.valor_receb) from recebimentos where recebimentos.cod_caixa = caixa.cod_caixa);

#11
update caixa set valordébitos_caixa = valordébitos_caixa + (select sum(pagamentos.valor_pag) from pagamentos where pagamentos.cod_caixa = caixa.cod_caixa);

#12
update caixa set saldofinal_caixa = saldoinicial_caixa +(valorcréditos_caixa - valordébitos_caixa);
update caixa set saldofinal_caixa = saldoinicial_caixa +(select sum(valorcréditos_caixa) - (select sum(valordébitos_caixa)));
##???????????